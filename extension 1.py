...
def page_acceuil():
    ...
    def choisir_nombre_couleurs(mode_de_jeu):
       for widget in fenetre.winfo_children():
           widget.destroy()
       fenetre.configure(bg = couleur_fond)
       ...
       def lancer_jeu_avec_nb_couleurs_choisi(nb_couleurs):
           if mode_de_jeu == "mode_solo":
               ouvrir_jeu(fenetre, mode_de_jeu, nb_couleurs)
           elif mode_de_jeu == "mode_duo":
               ouvrir_fenetre_choix_couleur(fenetre, nb_couleurs)
...
def ouvrir_jeu(fenetre,mode_de_jeu):
    ...
    def verifie_couleur_solo():
        ...
        """
        nonlocal nb_essai, code_secret, essai, index_rond, petits_ronds, erreur_label1, temp_essai,aide_label,frame_bouton,couleur_mauvaise_posi,couleur_bien_place,sauvegarde_essai
        aide_label.destroy()
    ...
    def valider_essai_joueur1():
        ...
        """
        nonlocal verification_en_cours, erreur_label1,canvas_gauche,frame_bouton,aide_label
        aide_label.destroy()
        frame_bouton.pack_forget()
        if len(essai) != 4:
            erreur_label1.pack(pady=360)
    ...
    def valider_verification_joueur2():
        ...
        frame_bouton.pack(padx=20,pady=670)
        nb_essai -= 1
        index_rond = 0
        sauvegarde_essai.append(essai)
        essai = []
    ...
    frame_aide.pack()
    frame_bouton = tk.Frame(canvas_gauche,bg= "white")
    frame_bouton.pack(padx=20,pady=670)
    aide_label = tk.Label(frame_aide, text="Remplissez les 4 couleurs :\nVert,Bleu,Rose,Jaune.", fg="white", font=("Arial", 15),bg="white")
    aide_label.pack()
    ...
    def aide():
        ...
        nonlocal nb_essai, combi_encore_possible, aide_label
        nonlocal couleurs,combinaison_optimal
        ...
        if nb_essai == 9:
            aide_label = tk.Label(frame_aide, text="Remplissez les 4 couleurs :\nVert,Bleu,Rose,Jaune.", fg="green", font=("Arial", 15),bg="white")
            aide_label.pack()
        ...
        aide_label = tk.Label(frame_aide, text=f"Remplissez les 4 couleurs :\n{aide_texte}", fg="green", font=("Arial", 15),bg="white")
        aide_label.pack()
    ...
    def IA():
        nonlocal nb_essai, combinaison_optimal,couleur_bien_place
        nonlocal temp_essai, combi_encore_possible,couleur_mauvaise_posi
        aide()

        for i in combinaison_optimal:
            colorer_rond(i, nb_essai)
            
        verifie_couleur_solo()

        if couleur_bien_place==4:
            return
        else:
            IA()
    ...
    def charger_sauvegarde():
        nonlocal code_secret, sauvegarde_essai, nb_essai, essai, index_rond
        ...
        nb_essai = 9
        for essai_i in sauvegarde_essai:
